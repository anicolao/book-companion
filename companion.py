...
class CompanionApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Container(
            Label(f"Book Companion - POC v0.1", id="title"),
            id="status-container"
        )
        ...
        yield Horizontal(
            Button("▶ Play" if not self.is_playing else "⏸ Pause", id="play-btn", variant="primary"),
    ...
    def _get_book_info(self) -> str:
        ...
        return f"Now reading: {self.book_title}\nPosition: Paragraph {self.current_paragraph_index + 1} of {len(self.paragraphs)}"
